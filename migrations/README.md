...
12. Apply `016_add_words_conjugations.sql` for the Practice hub's Conjugation Drill, then deploy `generate-conjugations`/`batch-generate-conjugations` (`supabase/functions/README.md`) and run `npm run backfill:conjugations-dry-run`, then `npm run backfill:conjugations-live` (see `docs/practice-hub-spec.md` "Deploy steps").
...
16. `016_add_words_conjugations.sql` — adds `words.conjugations` (jsonb, nullable): every personal tense of a French verb, generated by the `generate-conjugations` Edge Function from the rule-based `french-verbs` library + Lefff dictionary (no LLM) after a word is added or edited. The function also tags a detected verb's `part_of_speech` as `'verb'` when it had none. The full shape is documented in the migration's own comment and in `docs/practice-hub-spec.md`.
...
- `backfillConjugations.ts` — one-time sweep for `016`: runs verb detection over every existing French word with no conjugations yet (`npm run backfill:conjugations-dry-run` lists the words it would tag as verbs without writing; `npm run backfill:conjugations-live` writes).
