 ...
 * against the known (german, english) pairs is exact and free, so it's
 ...
 * truth). This is a keyword/diacritic guess, not a real language detector,
 * and is expected to be wrong on short, marker-free Spanish words -- there
 * is no network/ML language-detection call available in this environment.
...
export function detectTranslationLanguage(germanText: string, englishText: string): 'es' | 'en' {
  if (KNOWN_SEED_PAIRS.has(`${germanText}|||${englishText}`)) return 'es';
  return SPANISH_MARKERS.test(englishText) ? 'es' : 'en';
}
